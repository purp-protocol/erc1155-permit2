...
contract ERC1155SignatureTransfer is IERC1155SignatureTransfer, EIP712 {
	...
	if (requestedTokenId != permit.permitted.tokenId) revert InvalidTokenId(permit.permitted.tokenId);

        _useUnorderedNonce(owner, permit.nonce);

        signature.verify(_hashTypedData(dataHash), owner);

        IERC1155(permit.permitted.token).safeTransferFrom(owner, transferDetails.to, requestedTokenId, requestedAmount);
    }
    ...
    function _permitTransferFrom(
		...
		if (requestedTokenId != permitted.tokenId) revert InvalidTokenId(permitted.tokenId);

                if (requestedAmount != 0) {
                    // allow spender to specify which of the permitted tokens should be transferred
                    IERC1155(permitted.token).safeTransferFrom(owner, transferDetails[i].to, requestedTokenId, requestedAmount);
                }
            }
        }
    }
